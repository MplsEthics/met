...
class Scenario(db.Model):
    ...
    # Note: Scenario property 'answer_set' is autogenerated by the
    # ReferenceProperty in the Answer.
...
def populate_datastore():
    from met.content import testbank
    for scenario in testbank.values():
        sd = dict(scenario.__dict__)
        del sd['answers']
        del sd['answer_dict']
        ss = Scenario(**sd)
        ss.put()
        for answer in scenario.answers:
            ad = dict(answer.__dict__)
            aa = Answer(**ad)
            aa.scenario = ss
            aa.put()
